...
export default () => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < images.length; i++) {
            imagesEl[i].style.transform = `rotateY(${i * deg}deg) translateZ(640px)`
    ...
    const play = () => {
        if (playing) {
            rotateY++
            wrap.current!.style.transform = `rotateX(${rotateX}deg) rotateY(${rotateY}deg)`
        }
    ...
    return (
        <div className="w-screen h-screen overflow-hidden bg-slate-900">
            ...
            >
                ...
                <a className="w-[1200px] h-[1200px] rounded-full absolute top-full left-1/2 ml-[-600px] mt-[-600px]"></a>
            </div>
        </div>
    )
}
